...
export default function Financial({ moveToTab }) {
  ...
  const formik2 = useFormik({
    ...
    onSubmit: (val) => {
      ...
      if (editIndex == null) {
        setAllDebts((prev) => [...prev, formik.values]);
        formik.resetForm();
        setModalOpen2(false);
      } else {
        const currentDebt = [...allDebts];
        currentDebt[editIndex] = formik.values;
        setAllDebts(currentDebt);
        formik.resetForm();
        setModalOpen2(false);
        setEdit(null);
      }
                      ...
                      <tr key={ind.toString()}>
                        ...
                        <td>
                          ...
                          <FaPencilAlt
                              onClick={() => {
                                setIsOpen(true);
                                console.log(financial_dept_info[ind]);
                                // return
                                formik.setValues({
                                  ...formik.values[ind],
                                  initialValues: {
                                    financial_dept_info: {
                                      project_name:  addDebt[ind].financial_dept_info.project_name,
                                      location:  addDebt[ind].financial_dept_info.location,
                                      sector:  addDebt[ind].financial_dept_info.sector,
                                      evidence_of_support:  addDebt[ind].financial_dept_info.evidence_of_support,
                                      aggregate_amount:  addDebt[ind].financial_dept_info.aggregate_amount,
                                      date_of_financial_close:  addDebt[ind].financial_dept_info.date_of_financial_close,
                                      date_of_first_drawdown:  addDebt[ind].financial_dept_info.date_of_first_drawdown,
                                      date_of_final_drawdown:  addDebt[ind].financial_dept_info.date_of_final_drawdown,
                                      tenor_of_financing:  addDebt[ind].financial_dept_info.tenor_of_financing,
                                      borrower: {
                                        name:  addDebt[ind].borrower.name,
                                        rc_number:  addDebt[ind].borrower.rc_number,
                                        address:  addDebt[ind].borrower.address,
                                      },
                                    },
                                  }
                                });
