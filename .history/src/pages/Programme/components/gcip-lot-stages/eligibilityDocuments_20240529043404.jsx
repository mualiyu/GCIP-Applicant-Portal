...
function EligibilityDocuments({ saveData, nextRun }) {
  ...
  const [stageDocToUpload, setStageDocToUpload] = useState([]);
  const allDocs = [
    {
  ...
  const getData = async () => {
    ...
    if (response.success) {
      if (response.data.data.application.application_documents.length) {
        setStarted(true);
        setStageDocToUpload(
          response.data.data.application.application_documents
        );
        const uploaded = [];
        const notUploaded = [];
        allDocs.filter((data) => {
            ...
            onChange={(e) => {
              ...
              fetch(
                ...
                .then((data) => {
                  ...
                  if (data.status) {
                    newlyUploaded.push(data);
                    setStageDocToUpload(newlyUploaded);
                    setNewUpload(true);
                    // formik.handleSubmit();
                    setModalOpen2(false);
                  ...
                  console.log(setStageDocToUpload);
                  setTimeout(() => {
                    setAlert("");
                  }, 2000);
                })
