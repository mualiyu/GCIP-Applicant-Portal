...
function EligibilityDocuments({ saveData, nextRun }) {
  ...
  useEffect(() => {
    ...
    if (!started) {
      setNotUploaded(allDocs);
      const list = allDocs.map((doc) => doc.name);
      setNotUploadedSelect(list);
      return;
    }
    const newArray = allDocs.filter((doc) => {
      return !Uploaded.some((uploadedDoc) => uploadedDoc.name === doc.name);
    });
    setNotUploaded(newArray);
    const list = newArray.map((doc) => doc.name);
    setNotUploadedSelect(list);
  }, [started, Uploaded]);
  ...
  const handleFileUpload = async (file, fileName) => {
    ...
    try {
      ...
      setLoading(false);
      if (data.status) {
        const updatedUpload = { name: fileName, url: data.data.url };
        setUploaded((prev) => [...prev, updatedUpload]);
        setNotUploaded((prev) => prev.filter((doc) => doc.name !== fileName));
        setSelectedName("");
        formik.setFieldValue("document", [...Uploaded, updatedUpload]);
        setModalOpen2(false);
      } else {
      ...
      setLoading(false);
      setAlert("File upload failed. Please try again.");
      setTimeout(() => {
        setAlert("");
      }, 2000);
  ...
  return (
    <div>
      ...
      <Header
        ...
        style={{ fontSize: 12, color: "#9c0e0e" }}
      />
      <Input
      ...
      <table className="home_table">
        ...
        <tbody>
          <>
            {Uploaded.length > 0 &&
              Uploaded.map((doc, ind) => (
                <tr key={ind.toString()}>
                  <td>{ind + 1}</td>
                  <td>{doc.name}</td>
                  <td>{doc.url === "" ? "NOT-UPLOADED" : "UPLOADED"}</td>
                  <td>
                    <DeleteButton
                      label=""
                      onClick={() => {
        ...
        <div
          ...
          <Input
            ...
            onChange={(e) => {
              ...
              if (file) {
                handleFileUpload(file, selectedName);
              }
            }}
            label="Select File"
          />
