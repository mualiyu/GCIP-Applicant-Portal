...
export default function CompanyInfo() {
  ...
  const handleCorporateProfileChange = (event) => {
    const value = event.target.value;
    setCorporateProfile(value);
    localStorage.setItem("corporateProfile", JSON.stringify(value));
  };

  const handleMediumTermObjectivesChange = (event) => {
    const value = event.target.value;
    setMediumTermObjectives(value);
    localStorage.setItem("mediumTermObjectives", JSON.stringify(value));
  };

  const handleLongTermObjectivesChange = (event) => {
    const value = event.target.value;
    setLongTermObjectives(value);
    localStorage.setItem("longTermObjectives", JSON.stringify(value));
  };

  const handleProductDescriptionChange = (event) => {
    const value = event.target.value;
    setProductDescription(value);
    localStorage.setItem("productDescription", JSON.stringify(value));
  };

  const handleShortTermObjectivesChange = (event) => {
    const value = event.target.value;
    setShortTermObjectives(value);
    localStorage.setItem("shortTermObjectives", JSON.stringify(value));
  };

  const handleNumberOfStaffChange = (event) => {
    const value = event.target.value;
    setNumberOfStaff(value);
    localStorage.setItem("numberOfStaff", JSON.stringify(value));
  };

  useEffect(() => {
    console.log(localStorage.getItem("AppId"));
    const storedCorporateProfile = getLocalStorage("corporateProfile", "");
    const storedMediumTermObjectives = getLocalStorage(
      "mediumTermObjectives",
      ""
    );
    const storedLongTermObjectives = getLocalStorage("longTermObjectives", "");
    const storedProductDescription = getLocalStorage("productDescription", "");
    const storedShortTermObjectives = getLocalStorage(
      "shortTermObjectives",
      ""
    );
    const storedNumberOfStaff = getLocalStorage("numberOfStaff", "");
    setCorporateProfile(storedCorporateProfile);
    setMediumTermObjectives(storedMediumTermObjectives);
    setLongTermObjectives(storedLongTermObjectives);
    setProductDescription(storedProductDescription);
    setShortTermObjectives(storedShortTermObjectives);
    setNumberOfStaff(storedNumberOfStaff);
  ...
  const payload = {
    ...
    short_term_objectives: shortTermObjectives,
        ...
        <section style={{ display: "flex" }}>
          <div
            ...
            <input
              ...
              value={numberOfStaff}
              onChange={(e) => {
                handleNumberOfStaffChange(e.target.value);
              }}
              placeholder=""
