  ...
  useEffect(() => {
    const fetchInitialValues = async () => {
      ...
      const response = await query({
        method: "GET",
        url: "/api/applicant/application/get/business_proposal",
        token: programData.user.user.token,
      });

      if (response.success) {
        formik.setValues(response.data); // Assuming response.data has the required fields
      }
    };

    fetchInitialValues();
  }, [programData.user.user.token, formik.setValues]);
