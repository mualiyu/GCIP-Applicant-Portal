...
export default function Tab1({ moveToTab }) {
  ...
  const handleSelectedLot = (val) => {
    console.log(val);
    setLot(val);
    navigate(`/Programme/Application/${val.id}/continue`, { state: { selectedLot: val } );
  };

  const onLotSelection = async (lotID, lotName, reason) => {
    console.log("hey");
    setLoading(true);
    let selection = {
      update: "0",
          ...
          <section className="">
            ...
            <div
              ...
              <button
                ...
                onClick={() => onLotSelection(lot?.id, lot?.name, reason)}
                disabled={loading}
                style={{
                  fontSize: 9,
                  fontWeight: 900,
                  padding: "10px 40px",
                  backgroundColor: "#0c0c44",
                  marginLeft: 10,
                }}>
                {/* {loading ? "Wait..." : "Save"} */}
                Continue
