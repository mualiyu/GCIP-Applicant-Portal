...
export default function ReviewAndSubmit() {
  ...
  function checkIfRequirementsUploaded(project) {
    const { project_requirements, applicant_uploaded_documents } = project;
    ...
    if (!uploadedDocuments || !Array.isArray(uploadedDocuments)) {
      return false; // Handle the case where uploadedDocuments is undefined or not an array
    }
    return uploadedDocuments.every(
                  ...
                  <Table sx={{ minWidth: 650 }}>
                    ...
                    <TableBody>
                      {project?.project_requirements?.map((req, index) => {
                        const isRequirementUploaded =
                          checkIfRequirementsUploaded(
                            req?.id,
                            req?.applicant_uploaded_documents
                          );
